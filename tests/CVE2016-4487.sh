...
if [ ! -e obj-4487-patch ]; then 
	echo "Compile patched version."
	patch -p0 < ../patch-4487
	mkdir obj-4487-patch; cd obj-4487-patch
...
#### make clean any history file
##MK_DIR="ls ./"
##for i in $MK_DIR;
##do if [ -d $i ]; then
	##cd $i; make distclean; echo "make distclean in ${i}"
	##fi
##done
##rm ./*/config.cache
...
echo "First Compile."
#rm -rf obj-4487-1
#mkdir obj-4487-1; cd obj-4487-1;
#../configure --disable-shared --disable-gdb --disable-libdecnumber --disable-readline --disable-sim --disable-ld
#make 
echo "First Compile done."

cd $SUBJECT
TARGET=BUILD/obj-4487-1/binutils/cxxfilt
...
echo "Second compile."
rm -rf obj-4487-2
mkdir obj-4487-2; cd obj-4487-2; 
../configure --disable-shared --disable-gdb --disable-libdecnumber --disable-readline --disable-sim --disable-ld
make 
echo "Second compile done."

cd $SUBJECT 
TARGET=BUILD/obj-4487-2/binutils/cxxfilt
TIME=1m
...
do
	...
	#### valgrind ./BUILD/obj-4487-2/binutils/cxxfilt < ./crashfile
done
!
popd
