...
DOWNLOAD_DIR=$WORK/CVE-2011-3328
cd $DOWNLOAD_DIR
...
echo -e "pngrutil.c:1040\npngrutil.c:1042\npngrutil.c:1044" >$TMP_DIR/BBtargets.txt 
...
TARGET=target
rm ${TARGET}_profiled*
$CXX $CXXFLAGS $LDFLAGS -std=c++11 -v  $TEST_SUITE_DIR/libpng-1.2.56/target15.cc $TEST_SUITE_DIR/examples/example-hooks.cc $DOWNLOAD_DIR/BUILD/.libs/libpng15.a -I $DOWNLOAD_DIR/BUILD/ -lz -o ${TARGET}_profiled
...
do
	...
	/usr/bin/time -a -o $TIME_RECORD_FILE $AFLGO/afl-fuzz -S ${TARGET}_result -z exp -c $TIME -i $DIR_IN -o $DIR_OUT -m none -x $AFLGO/dictionaries/png.dict $SUBJECT/${TARGET}_profiled @@
